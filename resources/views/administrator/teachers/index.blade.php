...
<div class="container py-4">
    ...
    <button type="submit" class="btn btn-outline-danger btn-sm">Logout</button>
        ...
        <a href="{{ route('administrator.teachers.create') }}" class="btn btn-outline-success btn-sm">Criar</a>
    </div>
            ...
            <tbody>
                @foreach ($teachers as $teacher)
                    <tr>
                        <td>{{ $teacher->name }}</td>
                        <td class="text-end">
                            <a href="{{ route('administrator.teachers.edit', $teacher->id) }}" class="btn btn-primary btn-sm me-2">Editar</a>
                            <form action="{{ route('administrator.teachers.destroy', $teacher->id) }}" method="POST" class="d-inline"
