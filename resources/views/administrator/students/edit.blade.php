...
<body>
    ...
    <form method="POST" action="{{ route('administrator.students.update', $student->id) }}">
        ...
        <input type="text" id="phone" name="phone" maxlength="15" value="{{ $student->phone }}" required><br><br>

        <label>Email</label>
        <input type="email" name="email" value="{{ $student->email }}"><br><br>

        <label>Idiomas</label><br>
        @php $selectedLanguages = is_array($student->languages) ? $student->languages : explode(',', $student->languages); @endphp
        ...
        <input type="checkbox" name="languages[]" value="portugues"
            ...
            @php
                $selectedAvailability = is_array($student->availability) ? $student->availability : explode(',', $student->availability);
            @endphp
            <input type="checkbox" name="availability[]" value="manha" {{ in_array('manha', $selectedAvailability) ? 'checked' : '' }}> Manhã
